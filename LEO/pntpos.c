...
* history : 2010/07/28 1.0  moved from leocmn.c
*                           changed api:
*                               pntpos()
*                           deleted api:
*                               pntvel()
...
*           2011/11/08 1.2  enable snr mask for single-mode (leolib_2.4.1_p3)
                     ...
                     int iter, const prcopt_t *opt, double *var)
{
    ...
    *var=0.0;
    ...
    if (P1_P2==0.0&&(sys&(SYS_GPS|SYS_GAL))) {
...
{
    trace(6,"ionocorr: time=%s opt=%d sat=%2d pos=%.3f %.3f azel=%.3f %.3f\n",
...
{
    trace(6,"tropcorr: time=%s opt=%d pos=%.3f %.3f azel=%.3f %.3f\n",
                   ...
                   double *resp, int *ns)
{
    ...
    for (i=*ns=0;i<n&&i<MAXOBS;i++) {
        ...
        trace(3,"sat=%2d azel=%5.1f %4.1f res=%13.3f P=%13.3f r=%13.3f dtr=%13.3f dts=%13.3f trop=%7.3f ion=%7.3f sig=%5.3f\n",obs[i].sat,
              azel[i*2]*R2D,azel[1+i*2]*R2D, resp[i],P,r,dtr, CLIGHT*dts[i * 2],dtrp,dion,sqrt(var[nv-1]));
    }
	...
	//trace(3, "v=\t"); tracemat(3, v, 1, nv, 13, 4);
    return nv;
}
/* validate solution ---------------------------------------------------------*/
                  ...
                  char *msg)
{
    ...
    vv=dot(v,v,nv); 
    if (nv>nx&&vv>3*chisqr[nv-nx-1]) { //lei wang 
                  ...
                  double *resp, char *msg)
{
    ...
    for (i=0;i<MAXITR;i++) {
        ...
        if (norm(dx,NX)<1E-4) {
			...
			for (j = 0; j < 3; j++) sol->rp[j] = sol->rr[j];
                  ...
                  const double *azel, const int *vsat, double *v, double *H)
{
    ...
    for (i=0;i<n&&i<MAXOBS;i++) {
        ...
        /* range rate with earth rotation correction */  //rs  satellite positions and velocities (ecef)  
...
{
    prcopt_t opt_=*opt;
    double *rs,*dts,*var,*azel_,*resp,*dant,*rel;
    ...
    satposs(0,sol->time,obs,n,nav,opt_.sateph,rs,dts,var,svh, dant, rel);
