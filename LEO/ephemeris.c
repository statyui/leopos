...
* history : 2010/07/28 1.1  moved from leocmn.c
*                           added api:
...
static eph_t *seleph(gtime_t time, int sat, int iode, const nav_t *nav)
{
    ...
    trace(6,"seleph  : time=%s sat=%2d iode=%d\n",time_str(time,3),sat,iode);
...
{
    trace(6,"satpos  : time=%s sat=%2d ephopt=%d\n",time_str(time,3),sat,ephopt);
    
    *svh=0;
    
    switch (ephopt) {
        case EPHOPT_BRDC  : return ephpos     (time,teph,sat,nav,-1,rs,dts,var,svh);
        case EPHOPT_PREC  : if (!peph2pos(time,sat,nav,1,rs,dts, dant, rel, var)) break; else return 1;
    }
    *svh=-1;
    return 0;
}
...
*-----------------------------------------------------------------------------*/
extern void satposs(int post,gtime_t teph, const obsd_t *obs, int n, const nav_t *nav,
                    int ephopt, double *rs, double *dts, double *var, int *svh,  double *dant, double *rel)
{
	...
	if (post == 1) {
		for (i = 0; i < n&&i < MAXOBS; i++) {
			trace(4, "satposs: %s %d sat=%2d rs=%13.3f %13.3f %13.3f dts=%12.3f var=%7.4f dant=%8.4f %8.4f %8.4f rel=%13.8f svh=%02X\n",
				time_str(time[i], 6), post,obs[i].sat, rs[i * 6], rs[1 + i * 6], rs[2 + i * 6],
				dts[i * 2] * CLIGHT, var[i],dant[i*3],dant[i*3+1],dant[i*3+2],rel[i], svh[i]);
		}
	}
}
