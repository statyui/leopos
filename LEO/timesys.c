...
extern double time2mjd(gtime_t t)
{
	gtime_t t0 = epoch2time(gpst0);
	time_t sec = t.time - t0.time;
	return (sec + t.sec) / 86400 + 2444244.5;
}
/* add time --------------------------------------------------------------------
...
* notes  : ignore slight time offset under 100 ns
*-----------------------------------------------------------------------------*/
extern double getleaps(gtime_t t)
{
	gtime_t tu;
	int i;
	...
	for (i = 0; leaps[i][0]>0; i++) {
		tu = timeadd(t, leaps[i][6]);
		if (timediff(tu, epoch2time(leaps[i])) >= 0.0) return leaps[i][6];
	}
	return 0;
}
/* utc to gpstime --------------------------------------------------------------
...
{   
	...
	gtime_t tut, tut0;
	double ut, t1, t2, t3, t0, gmst0, gmst;

	tut = timeadd(t, ut1_utc);                                      
	ut = time2sec(tut, &tut0);                                      
	t0 = timediff(tut0, epoch2time(ep2000)) / 86400.0 / 36525.0;
	t1 = t0 + ut / 86400 / 36525.0;
	//t1 = timediff(tut, epoch2time(ep2000)) / 86400.0 / 36525.0;
	t2 = t1*t1; t3 = t2*t1;
	//modified by lei wang
	gmst0 = 24110.54841 + 8640184.812866*t0 + 0.093104*t2 - 6.2E-6*t3;
