...
*           2009/01/02  1.2  support new leo positioing api
...
*           2016/07/31  1.20 fix error message problem in rnx2leop
                      ...
                      const solopt_t *sopt)
{
    ...
    if (sopt->posf==SOLF_STAT) {
        return;
    }
    if (sopt->outhead) {
        if (!*sopt->prog) {
            fprintf(fp,"%s program   : LEOORB ver.%s\n",COMMENTH,VER_RTKLIB);
        }
        else {
    ...
    if (sopt->outopt&&sopt->posf!=SOLF_SP3) {
        outprcopt(fp,popt);
    }
...
/* initialize leo control ------------------------------------------------------
* initialize leo control struct
* args   : leo_t    *leo    IO  leo control/result struct
*          prcopt_t *opt    I   positioning options (see leoorb.h)
* return : none
*-----------------------------------------------------------------------------*/
extern void leoinit(leo_t *leo, const prcopt_t *opt)
{
	sol_t sol0 = { { 0 } };
	ambc_t ambc0 = { { { 0 } } };
	ssat_t ssat0 = { 0 };
	...
	leo->sol = sol0;
	for (i = 0; i<6; i++) leo->rb[i] = 0.0;
	leo->nx = pppnx(opt);
	leo->na = pppnx(opt);
	leo->tt = 0.0;
	leo->x = zeros(leo->nx, 1);
	leo->P = zeros(leo->nx, leo->nx);
	leo->xa = zeros(leo->na, 1);
	leo->Pa = zeros(leo->na, leo->na);
	leo->nfix = leo->neb = 0;
	for (i = 0; i<MAXSAT; i++) {
		leo->ambc[i] = ambc0;
		leo->ssat[i] = ssat0;
	}
	for (i = 0; i < 5; i++) leo->erpv[i] = 0.0;
	for (i = 0; i < 3; i++) leo->pcor[i] = 0.0;
	for (i = 0; i<MAXERRMSG; i++) leo->errbuf[i] = 0;
	leo->opt = *opt;
}
/* free leo control ------------------------------------------------------------
* free memory for leo control struct
* args   : leo_t    *leo    IO  leo control/result struct
* return : none
*-----------------------------------------------------------------------------*/
extern void leofree(leo_t *leo)
{
	trace(3, "leofree :\n");

	leo->nx = leo->na = 0;
	free(leo->x); leo->x = NULL;
	free(leo->P); leo->P = NULL;
	free(leo->xa); leo->xa = NULL;
	free(leo->Pa); leo->Pa = NULL;
}
                    ...
                    int mode)
{
    ...
    while ((nobs=inputobs(obs,leo.sol.stat,popt))>=0) {
...
#endif
        if (!leopos(&leo,obs,n,&navs)) continue;
        
        if (mode==0) { /* forward/backward */
            //if (!solstatic) {
                outsol(fp,&leo.sol,leo.rb,sopt);
            //}
            //else 
				if (time.time==0||pri[leo.sol.stat]<=pri[sol.stat]) {
                sol=leo.sol;
                for (i=0;i<3;i++) rb[i]=leo.rb[i];
                if (time.time==0||timediff(leo.sol.time,time)<0.0) {
                    time=leo.sol.time;
                }
            }
        }
            ...
            solf[isolf]=leo.sol;
            for (i=0;i<3;i++) rbf[i+isolf*3]=leo.rb[i];
            isolf++;
        }
        else { /* combined-backward */
            if (isolb>=nepoch) return;
            solb[isolb]=leo.sol;
            for (i=0;i<3;i++) rbb[i+isolb*3]=leo.rb[i];
            isolb++;
        }
    }
                      ...
                      obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    for (i=0;i<n;i++) {
		...
		/*read attitude file*/
		//strncpy(ext, infile[i] + strlen(infile[i]) - 3, 3);
		//for (j = 0; j < 3; j++) {
		//	if (ext[j] >= 'A'&&ext[j] <= 'Z') {
		//		ext[j] = ext[j] + 32;
		//	}
		//}
		fileext(infile[i], ext);
		if (strcmp(ext,"att")==0) { //read attitude file
			if (!(readatta(infile[i], &navs.att))) {
				showmsg("error : no leo attitude in %s", infile[i]);
				trace(1, "leo attitude file read error: %s\n", infile[i]);
				return 0;
			}
		}
		else if (strcmp(ext, "erp") == 0) {//read erp file
			free(navs.erp.data); navs.erp.data = NULL; navs.erp.n = navs.erp.nmax = 0;
			if (!readerp(infile[i], &navs.erp)) {
				showmsg("error : no erp data %s", infile[i]);
				trace(2, "no erp data %s\n", infile[i]);
			}
		}
        /* read rinex obs and nav file */
        else if (readrnxt(infile[i],rcv,ts,te,ti,prcopt->rnxopt[rcv<=1?0:1],obs,nav,
...
static void closeses(nav_t *nav, pcvs_t *pcvs, pcvs_t *pcvr)
{
	...
	free(nav->att.data); nav->att.data = NULL; nav->att.n = nav->att.nmax = 0;
	nav->geos.n = nav->geos.nmax = 0;
...
{
    int i,stat=0,flag=1,index[MAXINFILE]={0};
	FILE *fp;
	prcopt_t popt_ = *popt;
	...
	/* read gravity model file*/
	if (popt->mode>=PMODE_LEO_KIN) {
		reppath(fopt->geoid, path, ts, "", "");
		readgravity(path,popt->geomodel,&(navs.geos));
	}
