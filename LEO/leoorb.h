...
#define MAXSYS      4 /*max number of system*/
...
#define MAXGM		120					/* max gravity model order*/
...
#define PMODE_LEO_KIN    9               /* positioning mode: LEO-kinematic */
#define PMODE_LEO_RDN    10               /* positioning mode: LEO-reduce dynamic */
#define PMODE_LEO_FIX    11               /* positioning mode: LEO fixed */

#define SOLF_LLH    0                   /* solution format: lat/lon/height */
#define SOLF_XYZ    1                   /* solution format: x/y/z-ecef */
#define SOLF_ENU    2                   /* solution format: e/n/u-baseline */
#define SOLF_SP3    3                   /* solution format: NMEA-183 */
...
#define GEOMODEL_JGM3    0             /* geoid model: JGM3 */
#define GEOMODEL_EGM96   1             /* geoid model: EGM96 */
#define GEOMODEL_EGM2008 2             /* geoid model: EGM2008 */
#define GEOMODEL_EIGEN2   3             /* geoid model: EIGEN2*/
#define GEOMODEL_EIGEN_GL04C  4         /* geoid model: EIGEN_GL04C */
#define GEOMODEL_RAF09   5             /* geoid model: IGN RAF09 for France*/
...
typedef struct {        /* earth rotation parameter type */
	int n, nmax;         /* number and max number of data */
	int type;            /* geomodel type*/
	int maxorder;
	double radius;
	double gmconst;
	double C[MAXGM+1][MAXGM+1];
	double S[MAXGM+1][MAXGM+1];
} geo_t;
...
typedef struct {
	int n, nmax;         /* number of data/allocated */
	attd_t *data;         /* satellite attitude data */
}att_t;
...
typedef struct {        /* navigation data type */
	...
	att_t  att;         /* attitude of LEO satellite*/
	geo_t  geos;        /* geopotential file*/
...
typedef struct {        /* solution type */
	...
	double rp[3];         /* position of previous epochs*/
} sol_t;
...
typedef struct {        /* processing options type */
	...
	int geomodel;       /* geopotential model 0:JGM3,1:EGM96,2:egm08 3:EIGEN*/
	int geoorder;       /* geopotential order*/
...
typedef struct {        /* file options type */
	...
	char attp	[MAXSTRPATH]; /* leo attitude file*/
...
typedef struct {        /* RINEX options type */
    ...
    char mask[5][64];   /* code mask {GPS,GLO,GAL,CMP,IRN} */
    ...
    char tobs[5][MAXOBSTYPE][4]; /* obs types {GPS,GLO,GAL,CMP,IRN} */
    int nobs[5];        /* number of obs types {GPS,GLO,GAL,CMP,IRN} */
} rnxopt_t;
...
typedef struct {        /* satellite status type */
	...
	double trp;
	double pcos[3];
	double pcvs[NFREQ]; /* satellite pcv*/
	double pcvr[NFREQ]; /* receiver pcv*/
	double dg;          /* gravitation correction*/
	double tropmap;     /* tropsphere map function*/
...
EXPORT double getleaps(gtime_t t);
...
EXPORT int file_uncompress(const char *file, char *uncfile);
...
EXPORT int  peph2pos(gtime_t time, int sat, const nav_t *nav, int opt,
                     double *rs, double *dts, double *dant, double *rel, double *var);
...
EXPORT int  satpos(gtime_t time, gtime_t teph, int sat, int ephopt,
                   ...
                   int *svh, double *dant, double *rel);
EXPORT void satposs(int post,gtime_t time, const obsd_t *obs, int n, const nav_t *nav,
                    int sateph, double *rs, double *dts, double *var, int *svh, double *dant, double *rel);
...
EXPORT unsigned int leo_crc32  (const unsigned char *buff, int len);
EXPORT unsigned int leo_crc24q (const unsigned char *buff, int len);
EXPORT unsigned short leo_crc16(const unsigned char *buff, int len);
...
/* precise positioning -------------------------------------------------------*/
EXPORT void leoinit(leo_t *leo, const prcopt_t *opt);
EXPORT void leofree(leo_t *leo);
EXPORT int  openstat(const char *file, int level);
EXPORT void closestat(void);
EXPORT void outsolstat(leo_t *leo);
...
EXPORT void leoantoff(gtime_t time, const double *rs, const nav_t *nav, double *dant);
EXPORT double interppol(const double *x, double *y, int n);
EXPORT int p22kepler(double GM, double dt, const double *r_a, const double *r_b, double *kep);
EXPORT int kepler2pv(double GM, const double *kep, double dt, double *pv);
/*forces */
EXPORT double gravityJGM3(const double *r, double *U, double nmax, double mmax, double *a);
extern double gravityforce(const double *r, double *U, double nmax, double mmax, geo_t *geo, double *a);
EXPORT int readgravity(const char *file, int type, geo_t *geo);
...
EXPORT int fileext(const char *infile, char *ext);
