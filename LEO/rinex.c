...
static const int navsys[]={             /* satellite systems */
    SYS_GPS,SYS_GLO,SYS_GAL,SYS_CMP,0
};
...
static void convcode(double ver, int sys, const char *str, char *type)
{
	...
	else if (!strcmp(str, "L1")) { /* ver.2.11 GPS L1C,GLO L1C/A */
								   //if      (ver>=2.12) ; /* reject C1 for 2.12 */
		if (sys == SYS_GPS) sprintf(type, "%c1C", 'L');
		else if (sys == SYS_GLO) sprintf(type, "%c1C", 'L');
		else if (sys == SYS_GAL) sprintf(type, "%c1X", 'L'); /* ver.2.12 */
	}
    ...
    trace(6,"convcode: ver=%.2f sys=%2d type= %s -> %s\n",ver,sys,str,type);
}
/* decode obs header ---------------------------------------------------------*/
                           ...
                           int *flag, int *sats)
{
    ...
    trace(6,"decode_obsepoch: ver=%.2f\n",ver);
    ...
    trace(6,"decode_obsepoch: time=%s flag=%d\n",time_str(*time,3),*flag);
    return n;
}
/* decode obs data -----------------------------------------------------------*/
                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    if (!obs->sat) {
        trace(6,"decode_obsdata: unsupported sat sat=%s\n",satid);
        stat=0;
    }
    ...
    switch (satsys(obs->sat,NULL)) {
        ...
        case SYS_CMP: ind=index+3; break;
                      ...
                      char tobs[MAXOBSTYPE][4], sigind_t *ind)
{
    ...
    for (i=0;i<n;i++) {
        ...
        trace(6,"reject obs type: sys=%2d, obs=%s\n",sys,tobs[i]);
    }
    ind->n=n;
                       ...
                       char tobs[][MAXOBSTYPE][4], int *flag, obsd_t *data)
{
	...
	set_index(ver,SYS_CMP,opt,tobs[3],index+3);
                      ...
                      char tobs[][MAXOBSTYPE][4], obs_t *obs)
{
    ...
    trace(6,"readrnxobs: nobs=%d stat=%d\n",obs->n,stat);
                      ...
                      eph_t *eph)
{
    ...
    trace(6,"decode_eph: ver=%.2f sat=%2d\n",ver,sat);
                       ...
                       int *type, eph_t *eph, geph_t *geph)
{
    ...
    trace(6,"readrnxnavb: ver=%.2f sys=%d\n",ver,sys);
    ...
    while (fgets(buff,MAXRNXLEN,fp)) {
        ...
        if (i==0) {
            ...
            if (ver>=3.0||sys==SYS_GAL) { /* ver.3 or GAL */
                strncpy(id,buff,3);
                sat=satid2no(id);
                sp=4;
                       ...
                       obs_t *obs, nav_t *nav, sta_t *sta)
{
    ...
    /* uncompress file */
    if ((cstat=file_uncompress(file,tmpfile))<0) {
