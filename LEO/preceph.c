...
*           2015/05/10 1.15 modify api readdcb()
*-----------------------------------------------------------------------------*/
#include "leoorb.h"
...
extern double interppol(const double *x, double *y, int n)  //rs[i]=interppol(t,p[i],NMAX+1);     
{
    int i,j;
                      ...
                      double *dant)
{
    ...
    trace(6,"satantoff: time=%s sat=%2d\n",time_str(time,3),sat);
                    ...
                    double *rs, double *dts, double *dant, double *rel,double *var)
{
    ...
    trace(6,"peph2pos: time=%s sat=%2d opt=%d\n",time_str(time,3),sat,opt);
	...
	trace(6, "peph2pos: %s sat=%2d  dant=%13.8f %13.8f %13.8f rel=%13.8f\n", time_str(time, 3), sat, dant[0],dant[1],dant[2], 2.0*dot(rs, rs + 3, 3) / CLIGHT );
    return 1;
}
